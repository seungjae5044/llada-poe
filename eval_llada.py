...
class LLaDAEvalHarness(LM):
    ...
    def generate_until(self, requests):
        ...
        for batch in tqdm(batched_requests, desc="Generating..."):
            ...
            if self.use_cache:
                if self.dual_cache:
                    generated_answer, nfe = generate_with_dual_cache(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, block_length=self.block_length, temperature=0, remasking=self.remasking, mask_id=self.mask_id, threshold=self.threshold, factor=self.factor, reuse_topk=self.reuse_topk, topk_k=self.topk_k, alpha_base=self.alpha_base, invert=self.invert)
                else:
                    generated_answer, nfe = generate_with_prefix_cache(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, block_length=self.block_length, temperature=0, remasking=self.remasking, mask_id=self.mask_id, threshold=self.threshold, factor=self.factor, reuse_topk=self.reuse_topk, topk_k=self.topk_k, alpha_base=self.alpha_base, invert=self.invert)
            else:
                generated_answer, nfe = generate(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, block_length=self.block_length, temperature=0, remasking=self.remasking, mask_id=self.mask_id, threshold=self.threshold, factor=self.factor, reuse_topk=self.reuse_topk, topk_k=self.topk_k, alpha_base=self.alpha_base, invert=self.invert)
